...
public class Carte
{
    ...
    public Carte(SlaveServerConfig config) throws Exception
    {
        ...
        if (!Const.isEmpty(slaveServer.getPort()))
        {
            ...
            {
                LogWriter.getInstance().logError("Carte",
                        Messages.getString("Carte.Error.CanNotPartPort", slaveServer.getHostname(), "" + port), e);
                allOK = false;
            }
        }
        ...
        {
            final SlaveServer client = new SlaveServer("Dynamic slave [" + hostname + ":" + port + "]", hostname, ""
                    + port, slaveServer.getUsername(), slaveServer.getPassword());
            for (final SlaveServer master : config.getMasters())
            {
                ...
                try
                {
                    ...
                    LogWriter.getInstance().logBasic(
                            "Carte",
                            "Registered this slave server to master slave server [" + master.toString()
                                    + "] on address [" + master.getServerAndPort() + "]");
                } catch (Exception e)
                {
                    LogWriter.getInstance().logError(
                            "Carte",
                            "Unable to register to master slave server [" + master.toString() + "] on address ["
        ...
        if (allOK)
        {
            this.webServer = new WebServer(transformationMap, jobMap, socketRepository, detections, hostname, port,
                    config.isJoining());
        }
    }
    ...
    public static void main(String[] args) throws Exception
    {
        ...
        if (config == null)
        {
            ...
            System.err.println(Messages.getString("Carte.Usage.Example")
